...
function buildFromMessage(message) {
  ...
  if(message.From.EmailAddress.Address.match(/550 Relaying denied/i)) {
  ...
  document.metadata.to = (message.ToRecipients || []).reduce(function(prev, data) {
    if(data.EmailAddress.Address.match(/550 Relaying denied/i)) {
      return prev;
    }
...
module.exports = function uploadThread(conversationId, token, anyfetchClient, cb) {
  var document = {
    ...
    actions: {
      show: 'https://outlook.office365.com/owa/#path=/mail'
    },
    related: [],
      ...
      async.each(['inbox', 'sentitems'], function(folder, cb) {
        request('https://outlook.office365.com')
          ...
          .end(function(err, res) {
            ...
            messages[folder] = res.body.value || [];
            return cb(null);
          });
      }, function(err) {
        if(err) {
